class Rooms::RefreshesController < ApplicationController
  ...
  def show
    @new_messages = Bookmark.populate_for(@room.messages.with_threads.with_creator.with_canonical_includes.page_created_since(@last_updated_at))
    @updated_messages = Bookmark.populate_for(@room.messages.without(@new_messages).with_threads.with_creator.with_canonical_includes.page_updated_since(@last_updated_at))
  end
