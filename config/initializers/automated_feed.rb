module AutomatedFeed
  class Configuration
    ...
    def initialize
      @lookback_hours = ENV.fetch("AUTOMATED_FEED_LOOKBACK_HOURS", "168").to_i
      @max_conversations_per_scan = ENV.fetch("AUTOMATED_FEED_MAX_CONVERSATIONS", "50").to_i
      ...
      @room_scan_message_limit = ENV.fetch("AUTOMATED_FEED_ROOM_SCAN_MESSAGE_LIMIT", "500").to_i
      @room_scan_thread_limit = ENV.fetch("AUTOMATED_FEED_ROOM_SCAN_THREAD_LIMIT", "40").to_i
      @room_scan_context_backfill = ENV.fetch("AUTOMATED_FEED_ROOM_SCAN_CONTEXT_BACKFILL", "100").to_i
      @room_scan_lookback_hours = ENV.fetch("AUTOMATED_FEED_ROOM_SCAN_LOOKBACK_HOURS", "60").to_i
    end
  end
